...
def _(
    ...
    with mo.status.spinner(title="Generating answer...") as _spinner:
        rag_result = rag(db_manager=db_manager, question=question)

    query = rag_result['query']
    answer = rag_result['answer'].response
    was_cached = rag_result.get('cached', False)
...
## Task 1.1: Select few-shot examples dynamically based on their similarity to the user query 
## Purpose: So the LLM sees examples related to "Physics" "Cambridge" "Nobel Prize" -> helps Text2Cypher generate a more accurate Cypher query
@app.cell
def _(dspy, json):
    ...
    # create embeddings for all the questions
    ...
    class Embeddings(dspy.Module):
        ...
        def forward(self, query: str, k: int = None):
            ...
            # key logic: encode user question -> compute similarity -> return top k most similar examples
...
## Task 1.2: Defining what each LLM module should do
## Purpose:
@app.cell
def _(GraphSchema, Query, dspy):
    # Takes the full database schema and extract only the relevant parts for the user question
    ...
    # Takes (the question, the prunned schema, the dynamic few-shot exampels) and generates a Cypher query
    # Include many rules: (lowercase comparision, CONTAINS operator, correct property names, no APOC, etc.  )
    ...
    # Read the database ressults and write English (human-readable) answer
...
# Database manager (used in schema pruning and query generation)
@app.cell
def _(kuzu):
...
def _(
    ...
    class EnhancedGraphRAG(dspy.Module):
        ...
        def __init__(
            ...
            # Enhanced features (call from utils folder)
        ...
        def forward(self, db_manager: KuzuDatabaseManager, question: str):
            ...
            with self._track_stage("1_schema_retrieval"):
                ...
                print(f'EnhancedGraphRAG: Stage 1: Schema Retrieval {schema_dict}')

            with self._track_stage("2_cache_lookup"):
                cached_result = self.cache.get(question, schema_dict)
                print(f'EnhancedGraphRAG: Stage 2: Cache Lookup {cached_result}')
                if cached_result:
                    cached_result['cached'] = True
                    return cached_result

            with self._track_stage("3_schema_pruning"):
                prune_result = self.prune(question=question, input_schema=str(schema_dict))
                schema = prune_result.pruned_schema
                print(f'EnhancedGraphRAG: Stage 3: Schema Pruning {schema}')

            with self._track_stage("4_example_retrieval"):
                dynamic_examples = self.get_dynamic_examples(question, k=3)
                print(f'EnhancedGraphRAG: Stage 4: Example Retrieval {dynamic_examples}')
            ...
            with self._track_stage("5_text2cypher_generation"):
                ...
                print(f'EnhancedGraphRAG: Stage 5: Text2Cypher Generation {cypher_query}')
            ...
            with self._track_stage("6_query_validation_refinement"):
                ...
                print(f'EnhancedGraphRAG: Stage 6: Query Validation Refinement {cypher_query}')
            ...
            with self._track_stage("7_query_execution"):
                ...
                print(f'EnhancedGraphRAG: Stage 7: Query Execution {final_context}')            
            with self._track_stage("8_answer_generation"):
                ...
                print(f'EnhancedGraphRAG: Stage 8: Answer Generation {answer}')            
            response = {
                "question": question,
                "query": cypher_query,
                "answer": answer,
            ...
            with self._track_stage("9_cache_storage"):
                ...
                print(f'EnhancedGraphRAG: Stage 9: Cache Storage {response}')
...
def _():
    ...
    from utils.text2cypher_cache import Text2CypherCache
    from utils.cypher_validator import CypherQueryValidator, CypherSelfRefinement
    from utils.performance_benchmark import PerformanceTracker, create_text_visualization
